    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-4 offset-4 rounded bg-info mt-3 py-3">
                <h2 class="text-center fw-bold" style="font-size: 20px">Edit Data Produk {{ $event->id }}</h2>
                <form class="mt-3"
                    action="{{ route('dashboard.players.update', ['id' => $event->id]) }}"
                    ...
                    <div class="mb-1">
                        ...
                        <input type="text" class="form-control @error('nama') is-invalid @enderror" name="nama"
                            placeholder="Masukkan nama pemain" value="{{ old('nama') ? old('nama') : $event->name }}">
                        @error('nama')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="mb-1">
                        ...
                        <input type="number" class="form-control @error('berat') is-invalid @enderror" name="berat"
                            ...
                            value="{{ old('berat') ? old('berat') : $event->weight }}">
                        @error('berat')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="mb-1">
                        ...
                        <input type="number" class="form-control @error('harga') is-invalid @enderror" name="harga"
                            placeholder="Masukkan harga pemain"value="{{ old('harga') ? old('harga') : $event->price }}">
                        @error('harga')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="mb-1">
                        ...
                        <input type="number" class="form-control @error('stok') is-invalid @enderror" name="stok"
                            placeholder="Masukkan stok pemain"value="{{ old('stok') ? old('stok') : $event->stock }}">
                        @error('harga')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="mb-1">
                        ...
                        <select class="form-select form-control @error('kondisi') is-invalid @enderror"
                            ...
                            <option value="Bekas" {{ $event->condition == 'Bekas' ? 'Seleted' : '' }}>Bekas</option>
                            <option value="Baru" {{ $event->condition == 'Baru' ? 'Seleted' : '' }}>Baru</option>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="exampleFormControlTextarea1" class="form-label fw-semibold">Deskripsi</label>
                        <textarea class="form-control @error('deskripsi') is-invalid @enderror" name="deskripsi"
                            id="exampleFormControlTextarea1" rows="3" placeholder="Tuliskan deskripsi produk yang akan dijual">{{ old('deskripsi') ? old('deskripsi') : $event->description }}</textarea>
                        @error('harga')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="d-flex">
                        <div class="mx-auto">
                            <a href="{{ route('dashboard.events') }}" class="btn btn-warning me-2">
                                Kembali</a>
